...
### Backend
- **JWT 인증** — HMAC 서명 키 기반 Access/Refresh 토큰 분리 발급, `typ` 클레임으로 Access/Refresh 혼용 방지
- **Bedrock 챗봇** — 이전 대화 내역 + 뉴스 컨텍스트를 시스템 프롬프트에 결합. Rate Limit 검증 → 사용자 메시지 저장 → 히스토리 포함 AI 응답 생성 → 응답 저장 순으로 처리하며, 첫 메시지 시 세션 제목 자동 생성
- **Redis Rate Limit** — `유저ID:날짜` 키로 일일 호출 수 카운트, 자정 TTL 만료로 별도 스케줄러 없이 자동 초기화
- **S3 하이브리드 패턴** — 파일은 S3, 메타데이터는 DB로 분리 관리하여 DB 부하 절감. Presigned URL로 클라이언트가 S3에 직접 PUT 후 HeadObject로 업로드 검증
- **JPA 최적화** — `BaseEntity` 공통 Audit, 소프트 삭제(`is_deleted`), N+1 방지 fetch join, COALESCE 기반 null-safe 필터

### Frontend
- **axios 인터셉터** — 요청 시 토큰 자동 주입
- **Zustand + TanStack Query** — 전역 인증·국가 상태와 서버 상태 분리 관리
- **Kakao OAuth 콜백** — code를 백엔드에 전달 → JWT 저장 → 최초 로그인 여부에 따라 온보딩/메인 분기
- **낙관적 업데이트** — 채팅 메시지 전송 시 즉시 UI 반영

### Infrastructure
- **Terraform IaC** — 서울/도쿄 인프라를 코드로 정의하여 환경 일관성·재현성 확보 (Storage / Network / Ingress / EKS / DB 모듈 분리)
- **보안 계층 분리** — CloudFront OAC + SigV4로 S3 직접 접근 차단, WAF + ACM 전 구간 암호화, 최소 권한 기반 보안 그룹
